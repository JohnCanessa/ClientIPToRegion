...
public class BST {
    ...
    /**
     * Insert array values into the binary search tree..
     * 
     * Entry point for recursive call.
     */
    public TreeNode insert(String[] arr) {

        // **** loop through the array inserting nodes in the binary tree ****
        for (int i = 0; i < arr.length; i++)
            insert(this.root, new Region(arr[i]));
     ...
     * Recursive call.
     */
    public void insert(TreeNode node, Region region) {

        // **** tree is empty ****
        if (this.root == null) {
            this.root = new TreeNode(region);
            return;
        }

        // **** recurse left ****
        if (Integer.compareUnsigned(region.startHash, node.endHash) < 0) {

            // **** insert node here ****
            if (node.left == null) {

                // **** insert node here ****
                node.left = new TreeNode(region);
                return;
            }

            // **** keep on looking for a place to insert region ****
            insert(node.left, region);
        }

        // **** recurse right ****
        else if (Integer.compareUnsigned(region.endHash, node.endHash) > 0) {

            // **** ****
            if (node.right == null) {

                // **** insert node here ****
                node.right = new TreeNode(region);
                return;
            }

            // **** keep on looking for a place to insert region ****
            insert(node.right, region);
        }
    }
     ...
     * Recursion entry point.
     */
    public TreeNode find(TreeNode root, int hash) {

        // ???? ????
        // System.out.println("find <<< hash: " + Integer.toUnsignedString(hash));

        TreeNode[] found = new TreeNode[1];

        // **** start recursion ****
        find(root, hash, found);
     ...
     * Recursive call;
     */
    private void find(TreeNode root, int hash, TreeNode[] found) {

        if (root != null) {

            // **** traverse left subtree ****
            find(root.left, hash, found);

            // **** check if this is the node we are looking for ****
            // if (root.val == hash) {

            if ( Integer.compareUnsigned(hash, root.startHash) >= 0 &&
                (Integer.compareUnsigned(hash, root.endHash) <= 0)) {
                found[0] = root;
                return;
            }

            // *** traverse right subtree ****
            find(root.right, hash, found);
        }
